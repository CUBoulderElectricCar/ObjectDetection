...
# Determine color thresholds for color detection
...
def circle_color_detect():
    for key, value in upper.items():
            ...
            if len(cnts) > 0:
                ...
                if radius > 0.5:
                    cv2.circle(image_np, (int(x), int(y)), int(radius), colors[key], 2)
                    cv2.putText(image_np,key + " light", (int(x-radius),int(y-radius)), cv2.FONT_HERSHEY_SIMPLEX, 0.6,colors[key],2)
...
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
    while True:
      ...
      # Determine stop signs and traffic lights
      class_label = [category_index.get(i) for i in (classes[0])]
      print(class_label[0]['name'])
      if(class_label[0]['name'] == 'stop sign'):
          print('STOP')
      # If traffic light is detected, start circle color detection subroutine
      if(class_label[0]['name'] == 'traffic light'):
          circle_color_detect()

      #Destroy window if 'q' is pressed
